...
export default function IconPicker({ value, onChange, onClose }: IconPickerProps) {
  ...
  // 根据搜索和标签页变化时的重置键值
  const resetKey = useMemo(() => `${search}-${tab}`, [search, tab]);

  // 当前可见数量
  const [currentVisibleCount, setCurrentVisibleCount] = useState(120);

  // 当搜索或标签页变化时重置可见数量
  // 使用 setTimeout 避免在 effect 中同步调用 setState
  useEffect(() => {
    const timer = setTimeout(() => {
      setCurrentVisibleCount(120);
    }, 0);
  ...
  const handleScroll = useCallback((event: React.UIEvent<HTMLDivElement>) => {
    ...
    if (scrollHeight - scrollTop - clientHeight < 220) {
      ...
      setCurrentVisibleCount((previous) => Math.min(previous + 120, currentLength));
    }
  }, [filteredBrandIcons.length, filteredLucideIcons.length, tab]);
